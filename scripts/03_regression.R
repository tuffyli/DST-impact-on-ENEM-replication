...
# ---------------------------------------------------------------------------- #
#### 1.1.5.1 Stata bootstrap ----
# I will extract the main dataframe and bandwidth to further apply a robustness
# test for the clustering. The wild bootstrap test will be applied in stata.
# ---------------------------------------------------------------------------- #
...
df <- data.frame(
  ...
  x = base_res$dist_hv_res[base_res$ano == 2018],
  cluster = base_res$seg_res[base_res$ano == 2018],
  w = base_res$obs[base_res$ano == 2018],
  
  # controls (same order!)
  efr,
  lat = base_res$lat_res[base_res$ano == 2018],
  lon = base_res$lon_res[base_res$ano == 2018],
  
  #Weather
  dtempd1 = base_res$dtempd1[base_res$ano == 2019],
  dumidd1 = base_res$dumidd1[base_res$ano == 2019],
  dumidd2 = base_res$dumidd2[base_res$ano == 2019], #Humidty d2
  dtempd2 = base_res$dtempd2[base_res$ano == 2019], #temp2
  
  #House
  dn_ban    = base_res$dn_ban[base_res$ano == 2019], #bathrooms
  dpessoa   = base_res$dpessoa[base_res$ano == 2019], #people in household
  dn_qua    = base_res$dn_qua[base_res$ano == 2019], #houses
  dn_car    = base_res$dn_car[base_res$ano == 2019], #cars
  dn_gel    = base_res$dn_gel[base_res$ano == 2019], # refrigerator
  dn_cel    = base_res$dn_cel[base_res$ano == 2019], # cellphone
  dpc       = base_res$dpc[base_res$ano == 2019],    #pc
  dinternet = base_res$dinternet[base_res$ano == 2019], #internet
  dempr_dom = base_res$dempr_dom[base_res$ano == 2019], #Housekeeping
  
  #remaining
  descm     = base_res$descm[base_res$ano == 2019], #mother educ
  dfem      = base_res$dfem[base_res$ano == 2019], #Female
  dppi      = base_res$dppi[base_res$ano == 2019], #PPI
  didade    = base_res$didade[base_res$ano == 2019], #Age
  descp     = base_res$descp[base_res$ano == 2019], #father educ
  drenda1   = base_res$drenda1[base_res$ano == 2019], #wage < 1MW
  drenda110 = base_res$drenda110[base_res$ano == 2019], #wage 1MW - 10MW
  drenda10  = base_res$drenda10[base_res$ano == 2019], #wage > 10MW
  dgdppc    = base_res$dgdppc[base_res$ano == 2019], #gdppc
  
  #Timezone
  h13 = base_res$h13[base_res$ano == 2019],
  h12 = base_res$h12[base_res$ano == 2019],
  h11 = base_res$h11[base_res$ano == 2019]
) %>% 
  mutate(
    bw = bw_main_r
  ) %>% 
...
#Saving and removing the stata dataframe
write_dta(df, file.path(stata_output_path, "main_df_boot.dta"))
rm(df)
# ---------------------------------------------------------------------------- #
### 1.1.6 Pol + All ----
...
var_list <- c("id18", "fem", "ppi", "escp", "escm", # "dom5",
              ...
              "renda1", "gdppc",# "pai_trab_man", "mae_trab_man",
